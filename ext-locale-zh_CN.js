...
Ext.define("Ext.locale.zh_CN.form.field.Time", {
    ...
    format: "g:i:s",
    invalidText: "{0} 不是一个有效的日期!",
    formatDate: function (val) {
        if (!isNaN(val)) {
            val = new Date(parseInt(val));
            val = Ext.Date.format(val, this.format);
            this.setValue(val);
            return val;
        } else {
            return val;
...
Ext.define("Ext.locale.zh_CN.grid.GridPanel", {
    override: "Ext.grid.GridPanel",
    emptyText: "<center><span style='color:red;'>未找到匹配项!</span></center>"
});
